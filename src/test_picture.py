...
def camera2robot_base(pos):
	new_pos = [-pos[0]+0.5, pos[1]+0.5, 1.5 - pos[2]]
...
def coords_callback(floatArray):
	...
	target_trajectory_1 = [target_pose[0], target_pose[1], target_pose[2]+0.3]
	target_trajectory_2 = [target_pose[0], target_pose[1], target_pose[2]+0.2]
	target_trajectory_3 = [target_pose[0], target_pose[1], target_pose[2]+0.1]
	pose_msgs =  [utils.create_pose_msg(target_trajectory_1, ori), \
			...
			utils.create_pose_msg(target_pose, ori)]
	plan, f = r.plan_cartesian_path(pose_msgs)
...
def main():
	...
	rospy.Subscriber(coords_topic, Float64MultiArray, coords_callback)
	# Spin until ctrl + c
	rospy.spin()
